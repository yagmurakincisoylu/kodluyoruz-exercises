...
const menuElements = (title, price, img, desc) => {
  menuItems.innerHTML += `
    <div class="menu-items col-8 col-lg-6 mx-auto mx-lg-0">
...
const filterMenu = (arr) => {
  arr.forEach(value => {
    const {id, title, category, price, img, desc} = value;
    menuElements(title, price, img, desc);
  });
}
...
const filter = param => {
  ...
  if (param === "All") {
    ...
    const arr = menu.filter(user => user.category === param );
    filterMenu(arr);
  } 
